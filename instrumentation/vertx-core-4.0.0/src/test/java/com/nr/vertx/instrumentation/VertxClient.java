...
public class VertxClient {
    ...
    public void getCall() throws InterruptedException {
        ...
        httpClient.request(HttpMethod.OPTIONS, port, "localhost", "/")
                .compose(HttpClientRequest::send)
                .onComplete(requestHandler(latch));
        latch.await();
    }
    ...
    private void postCall() throws InterruptedException {
        CountDownLatch latch = new CountDownLatch(1);
        HttpClient httpClient = vertx.createHttpClient();
        httpClient.request(HttpMethod.POST, port, "localhost", "/")
                .compose(HttpClientRequest::send)
                .onComplete(requestHandler(latch));
        latch.await();
    }
    ...
    public void endMethods() throws InterruptedException {
        ...
        httpClient.request(HttpMethod.GET, port, "localhost", "/hi").onSuccess(t -> t.send(requestHandler(latch)));
        httpClient.request(HttpMethod.GET, port, "localhost", "/hi").onSuccess(t -> t.send(bufferChunk, requestHandler(latch)));
        httpClient.request(HttpMethod.GET, port, "localhost", "/hi").onSuccess(t -> t.send(stringChunk, requestHandler(latch)));
        httpClient.request(HttpMethod.GET, port, "localhost", "/hi").compose(t -> {
            t.response(requestHandler(latch));
            return t.end(stringChunk, encoding);
        });
        latch.await();
    }
    ...
    public void requestMethods() throws InterruptedException {
        ...
        httpClient.request(HttpMethod.GET, port, "localhost", "/hi").compose(HttpClientRequest::send).onComplete(requestHandler(latch));
        httpClient.request(HttpMethod.POST, port, "localhost", "/hi").compose(HttpClientRequest::send).onComplete(requestHandler(latch));
        httpClient.request(HttpMethod.PUT, port, "localhost", "/hi").compose(HttpClientRequest::send).onComplete(requestHandler(latch));
        httpClient.request(HttpMethod.HEAD, port, "localhost", "/hi").compose(HttpClientRequest::send).onComplete(requestHandler(latch));
        httpClient.request(HttpMethod.DELETE, port, "localhost", "/hi").compose(HttpClientRequest::send).onComplete(requestHandler(latch));
        latch.await();
    }
    ...
    public void redirect() throws InterruptedException {
        HttpClient httpClient = vertx.createHttpClient();
        CountDownLatch latch = new CountDownLatch(1);
        httpClient.request(HttpMethod.GET, port, "localhost", "/redirect")
                .map(r -> r
    ...
    public void cat(HttpTestServer httpServer) throws Exception {
        try {
            CountDownLatch latch = new CountDownLatch(1);
            HttpClient httpClient = vertx.createHttpClient();
            httpClient.request(HttpMethod.GET, httpServer.getEndPoint().getPort(), httpServer.getEndPoint().getHost(), "/")
                    .map(r -> r.putHeader(HttpTestServer.DO_CAT, "true"))
                    .compose(HttpClientRequest::send)
                    .onComplete(requestHandler(latch));
            latch.await();
        } finally {
    ...
    private void unknownHost() throws InterruptedException {
        ...
        httpClient.request(HttpMethod.GET, port, "notARealHostDuderina.com", "/")
                .map(r -> r.exceptionHandler(exceptionHandler(latch)))
                .compose(HttpClientRequest::send)
    ...
    private Handler<AsyncResult<HttpClientResponse>> requestHandler(CountDownLatch latch) {
        return result -> {
            if (result.succeeded()) {
                ...
                NewRelic.addCustomParameter("responseHandler", "false");
                latch.countDown();
                Assert.fail("Unhandled error: " + result.cause().getMessage());
            }
        };
    }
